# Astro 版 frontend2 再構築計画

## Summary

- `packages/yomo-yama-frontend2` に Astro の静的サイトとして再構築する。
- React/Preact/Vue/Svelte などの renderer は入れず、Astro コンポーネント、Markdown、ビルド時 TypeScript/JavaScript、通常のクライアント JS だけで実装する。
- 既存 Eleventy と URL 互換を優先し、`packages/yomo-yama-frontend` は比較用に残す。

## Key Changes

- 新規 workspace `yomo-yama-frontend2` を追加し、依存は `astro`, `sass`, `@astrojs/rss`, `@astrojs/sitemap`, `@sett4/yomo-yama-db` に絞る。
...
- `npm run build:db`
- `DATABASE_URL=file:../yomo-yama-db/local.db npm run build --workspace yomo-yama-frontend2`
- `npm run check --workspace yomo-yama-frontend2`
